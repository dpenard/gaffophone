...
#define GLCD_CONTROLLER_ST7565R 1
#define GLCD_INIT_NHD_C12832A1Z_FSW_FBW_3V3 1
...
void glcd_ST7565R_init(void) {
	...
	/* We use "reverse direction" for common output mode, as opposed to datasheet specifying "normal direction" */
		...
		glcd_command(0xae); /* Display OFF */
		glcd_command(0xa2); /* ADC select in normal mode */
		...
		glcd_command(0x22); /*33 LCD bias set at 1/9 */
		glcd_command(0x2f); /* Power control set to operating mode: 7 */
		glcd_command(0x40); /* Internal resistor ratio, set to: 1 */
		glcd_command(0xaf); /* ADC select in normal mode */
		glcd_set_contrast(0x17); /* Set contrast, value experimentally determined, can set to 6-bit value, 0 to 63 */
		glcd_command(0xa6); /* Display on */
...
//			glcd_set_contrast(63); /* Set contrast, value experimentally determined, can set to 6-bit value, 0 to 63 */
